...
void balance_control()
{
	...
	force1 = (front_leg_frame_height  - initial_front_leg_frame_height)*Kp_force*5;
	//force1  = (max(min(front_left[2], front_right[2]), FOOT_THICKNESS/2)-FOOT_THICKNESS/2)*Kp_force + 350;

	//force1  = (max(front_left[2], front_right[2])-FOOT_THICKNESS/2)*Kp_force*0.5 + 350;
	...
	*/			
	...
	force1 = (back_leg_frame_height  - initial_back_leg_frame_height)*Kp_force*10;
	
	
	//force1  = (max(min(back_left[2], back_right[2]),FOOT_THICKNESS/2)-FOOT_THICKNESS/2)*Kp_force + 350;
	...
	if (n>0)
	{
		...
		if (front_right_phase != 0)
		{
			...
			for(int i=12;i<15;i++)
			{
				...
				dBodyAddForce(robot_bodies[i], new_v_force*0.1, 0, 0);
			}
		}
		else
		{
			if (new_v_force>0)
			{
				...
				if (diff > 0.1)
				{
					...
					//dBodyAddForce(robot_bodies[2], 0, 0, -1000);
				}
				else
					dBodyAddForce(robot_bodies[2], -new_v_force/5, 0, 0);
		...
		else
		{
			...
			if (new_v_force>0)
			{
				...
				if (diff > 0.1)
					...
					dBodyAddForce(robot_bodies[0], -new_v_force/5, 0, 0);
...
void get_P2(dVector3 P1, const dReal *frame, dVector3 P2, int n)
{
	if (n < 0 )
	{
		if (walk_start_back==0)
		{
			...
			P2[0] = frame[0]  + FOOT_LENGTH/2 + 0.02;
		else
